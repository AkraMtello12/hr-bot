...
try:
    # محاولة قراءة بيانات الاعتماد من متغيرات البيئة (الأفضل للإنتاج)
    ...
    if firebase_creds_json:
        ...
        # إذا لم تكن موجودة، استخدم الملف المحلي (مناسب للتطوير)
    ...
    # الخروج من البرنامج إذا فشل الاتصال بقاعدة البيانات
    exit()
...
def create_advanced_calendar(year: int, month: int, selection_mode: str, selected_dates: list, back_callback: str) -> InlineKeyboardMarkup:
    ...
    # صف الرأس: التنقل بين الشهور والسنة
    header_row = [
        ...
        for day in week:
            if day == 0:
                ...
                if current_day in selected_dates:
                    day_text = f"*{day}*" # تمييز اليوم المحدد
...
def create_time_keyboard(leave_type: str, back_callback: str) -> InlineKeyboardMarkup:
    ...
    keyboard = []
    if leave_type == 'late':
        # أوقات الوصول المتأخر (9:30 - 14:00)
        keyboard = [
        ...
        # أوقات المغادرة المبكرة (11:00 - 15:30)
        keyboard = [
    ...
    # إضافة زر الرجوع والقائمة الرئيسية
    keyboard.append([InlineKeyboardButton("➡️ رجوع", callback_data=back_callback), InlineKeyboardButton("القائمة الرئيسية ↩️", callback_data="main_menu")])
    return InlineKeyboardMarkup(keyboard)
...
async def choose_hourly_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        text=message,
        reply_markup=create_time_keyboard(leave_type, back_callback="hl_back_to_type")
    )
...
async def select_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(f"تم اختيار الوقت: {selected_time}.")
    
    keyboard = [[InlineKeyboardButton("➡️ رجوع", callback_data="hl_back_to_time")], [InlineKeyboardButton("القائمة الرئيسية ↩️", callback_data="main_menu")]]
    await query.message.reply_text("الآن، يرجى إدخال اسمك الكامل لتوثيق الطلب:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def enter_hourly_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [[InlineKeyboardButton("➡️ رجوع", callback_data="hl_back_to_name")], [InlineKeyboardButton("القائمة الرئيسية ↩️", callback_data="main_menu")]]
    await update.message.reply_text("شكراً لك. يرجى الآن توضيح سبب هذا الإذن:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def enter_hourly_reason(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("➡️ رجوع", callback_data="hl_back_to_reason")]
    ]
...
async def start_full_day_leave(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text("لتسجيل طلب إجازة، يرجى إدخال اسمك الكامل:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def fd_enter_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data['employee_name'] = update.message.text
    keyboard = [[InlineKeyboardButton("➡️ رجوع", callback_data="fd_back_to_name")], [InlineKeyboardButton("القائمة الرئيسية ↩️", callback_data="main_menu")]]
    await update.message.reply_text("شكراً لك. الآن، يرجى توضيح سبب الإجازة:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def fd_enter_reason(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("➡️ رجوع", callback_data="fd_back_to_reason")],
        [InlineKeyboardButton("القائمة الرئيسية ↩️", callback_data="main_menu")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def show_fd_confirmation(query, context):
    ...
    keyboard = [
        [InlineKeyboardButton("✅ تأكيد وإرسال", callback_data="confirm_send"), InlineKeyboardButton("❌ إلغاء الطلب", callback_data="cancel")],
        [InlineKeyboardButton("➡️ رجوع", callback_data="fd_back_to_calendar")]
    ]
...
async def hr_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    leave_type_key = parts[1] # fd | hourly
    request_id = parts[2]
    ...
    if not leave_request:
        await query.edit_message_text("خطأ: هذا الطلب لم يعد موجوداً في قاعدة البيانات.")
        return

    # تم إزالة التحقق من حالة الطلب "pending" للسماح بمعالجة كل ضغطة كحدث جديد
...
def main() -> None:
    ...
    # --- تعريف معالجات الرجوع (Back Handlers) ---
    
    # رجوع في محادثة الإذن الساعي
    hl_back_to_type = CallbackQueryHandler(start_hourly_leave, pattern="^hl_back_to_type$")
    async def hl_back_to_time_wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        query.data = f"hourly_{context.user_data['hourly_leave_type']}"
        return await choose_hourly_type(update, context)
    hl_back_to_time = CallbackQueryHandler(hl_back_to_time_wrapper, pattern="^hl_back_to_time$")
    async def hl_back_to_name_wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        query.data = f"TIME_{context.user_data['selected_time']}"
        return await select_time(update, context)
    hl_back_to_name = CallbackQueryHandler(hl_back_to_name_wrapper, pattern="^hl_back_to_name$")
    
    # رجوع في محادثة الإجازة اليومية
    fd_back_to_name = CallbackQueryHandler(start_full_day_leave, pattern="^fd_back_to_name$")
    async def fd_back_to_duration_type_wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        # We need to simulate the message that led to this state
        fake_update = type('FakeUpdate', (), {'message': update.callback_query.message, 'text': context.user_data['leave_reason']})()
        return await fd_enter_reason(fake_update, context)
    fd_back_to_duration_type = CallbackQueryHandler(fd_back_to_duration_type_wrapper, pattern="^fd_back_to_duration_type$")
    async def fd_back_to_calendar_wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        query.data = f"duration_{context.user_data['duration_type']}"
        return await fd_choose_duration_type(update, context)
    fd_back_to_calendar = CallbackQueryHandler(fd_back_to_calendar_wrapper, pattern="^fd_back_to_calendar$")
        ...
        states={
            ...
            FD_CHOOSING_DURATION_TYPE: [
                CallbackQueryHandler(fd_choose_duration_type, pattern="^duration_"),
                 CallbackQueryHandler(fd_enter_name, pattern="^fd_back_to_reason$"), # Simulate back by re-entering name
            ],
            FD_SELECTING_DATES: [
                CallbackQueryHandler(fd_calendar_callback, pattern="^CAL_"),
                fd_back_to_duration_type
            ],
            FD_CONFIRMING_LEAVE: [
    ...
    hourly_leave_conv = ConversationHandler(
        ...
        states={
            ...
            HL_ENTERING_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, enter_hourly_name),
                hl_back_to_time,
            ],
            HL_ENTERING_REASON: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, enter_hourly_reason),
                 CallbackQueryHandler(select_time, pattern="^hl_back_to_name$")
            ],
            HL_CONFIRMING_LEAVE: [
                CallbackQueryHandler(confirm_hourly_leave, pattern="^confirm_send$"),
                CallbackQueryHandler(enter_hourly_name, pattern="^hl_back_to_reason$")
            ],
        },
        fallbacks=[
            CallbackQueryHandler(cancel_conversation, pattern="^cancel$"),
